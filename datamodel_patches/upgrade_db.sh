...
EXTDIRS=()
DELTADIRS=()
DELTADIRS+=/home/sitadmin/sit/production/qwat_upgrade_133_to_137/QWAT/data-model/update/delta
INITFILES=()
...
if [[ $CLEAN -eq 1 ]]; then
    ...
    DBCOMP_NAME=$(sed -n -e "/^\[pum_qwat_comp]/,/^\[/ p" $PGSERVICEFILE | grep "^dbname" | cut -d"=" -f2)
    DBCOMP_USER=$(sed -n -e "/^\[pum_qwat_comp]/,/^\[/ p" $PGSERVICEFILE | grep "^user" | cut -d"=" -f2)
    DBCOMP_HOST=$(sed -n -e "/^\[pum_qwat_test]/,/^\[/ p" $PGSERVICEFILE | grep "^host" | cut -d"=" -f2)
    DBCOMP_PORT=$(sed -n -e "/^\[pum_qwat_test]/,/^\[/ p" $PGSERVICEFILE | grep "^port" | cut -d"=" -f2)
    DBTEST_NAME=$(sed -n -e "/^\[pum_qwat_test]/,/^\[/ p" $PGSERVICEFILE | grep "^dbname" | cut -d"=" -f2)
    DBTEST_USER=$(sed -n -e "/^\[pum_qwat_test]/,/^\[/ p" $PGSERVICEFILE | grep "^user" | cut -d"=" -f2)
    DBTEST_HOST=$(sed -n -e "/^\[pum_qwat_test]/,/^\[/ p" $PGSERVICEFILE | grep "^host" | cut -d"=" -f2)
    DBTEST_PORT=$(sed -n -e "/^\[pum_qwat_test]/,/^\[/ p" $PGSERVICEFILE | grep "^port" | cut -d"=" -f2)
...
# be there for the main pum_qwat_prod database
printf "\n${BLUE}Initializing qwat comparison db${NC}\n\n"

sleep 1
${SCRIPTDIR}/../init_qwat.sh -p pum_qwat_comp -s $SRID

# Initialize qwat db with extensions/customizations
for i in "${INITFILES[@]}"
do
	echo $i
	$i -p pum_qwat_comp -s $SRID
done
...
#pum test-and-upgrade -pp pum_qwat_prod -pt pum_qwat_test -pc pum_qwat_comp -t qwat_sys.info -d delta/ -f $TMPFILEDUMP -i columns constraints views sequences indexes triggers functions rules
#pum test-and-upgrade -x -pp pum_qwat_prod -pt pum_qwat_test -pc pum_qwat_comp -t qwat_sys.info -d ${DELTADIRS[*]} -f $TMPFILEDUMP -i views rules

pum test-and-upgrade -x -pp pum_qwat_prod -pt pum_qwat_test -pc pum_qwat_comp -t qwat_sys.info -d ${DELTADIRS[*]} -f $TMPFILEDUMP -i views rules -N public -N chenyx06 -N usr_cartoriviera -N usr_sige
